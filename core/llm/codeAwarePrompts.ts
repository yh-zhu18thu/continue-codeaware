...
每个 step 的 abstract 需要**通俗、生动、可读**，长度**控制在 3-6 句话（60-150字）**，使用 Markdown 格式。
...
2. **点明为什么要这一步**：说清楚这步的作用或做完后的效果。
3. **自然流畅，适当展开**：不要分条罗列，不要使用"本步骤"、"对整体项目的贡献"这类书面腔。就像在口头给朋友解释一样自然地写，可以用 3-6 句话把来龙去脉讲清楚。超过6句话就是太长了。
...
- **严禁超过6句话**。如果你发现写了7句以上，请删减合并到3-6句
  ...
  "steps": [
    {
      ...
      "abstract": "<通俗的步骤描述，Markdown格式，3-6句话（60-150字）。用大白话讲清楚这步干什么和为什么重要，像跟没编程基础的朋友解释一样>",
...
4. 每个 step 的 abstract 是否在 3-6 句话（60-150字）？是否用大白话写清楚了这步做什么、为什么重要？如果超过6句话，必须删减合并后再输出。
...
3. **清晰且友好，适当展开**：总长度控制在 **100-200 字**。语气像一位耐心的老师在给零基础学生讲解——温和、清楚、不居高临下。每个功能域的描述可以稍加展开，帮助学习者建立对后续学习内容的预期。
4. **第二人称**：使用"你"来称呼学习者。
