...
export class CodeAwareLoggerService {
  ...
  private getWorkspaceDirectory(): string {
    ...
    try {
      ...
      console.warn(
        "[CodeAwareLogger] Failed to get current working directory:",
        error,
      );
    }
    ...
    console.warn(
      "[CodeAwareLogger] Using home directory as final fallback:",
      homeDir,
    );
    return homeDir;
  }
  ...
  public setWorkspaceDirectory(workspaceDir: string): void {
    if (workspaceDir && fs.existsSync(workspaceDir)) {
      ...
      console.warn(
        "[CodeAwareLogger] Invalid workspace directory:",
        workspaceDir,
      );
    }
  }
  ...
  public addLogEntry(eventType: string, payload: any): void {
    if (!this.currentSession || !this.currentLogFilePath) {
      console.warn(
        "[CodeAwareLogger] No active session. Cannot log event:",
        eventType,
      );
      return;
    }
  ...
  private ensureLogDirectoryExists(): void {
    if (!fs.existsSync(this.logDirectory)) {
      try {
        ...
        console.error(
          "[CodeAwareLogger] Failed to create log directory:",
          error,
        );
      }
    }
