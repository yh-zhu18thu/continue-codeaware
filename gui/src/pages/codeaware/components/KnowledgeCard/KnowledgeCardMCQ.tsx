...
const KnowledgeCardMCQ: React.FC<KnowledgeCardMCQProps> = ({
  ...
  const getOptionButtonClass = (option: string) => {
    const baseClasses =
      "rounded-lg p-3 w-full text-left border transition-all duration-300 ease-in-out transform focus:outline-none text-sm"; // 统一使用 text-sm

    // VSCode 风格的颜色变量 (假设这些 CSS 变量已在项目中定义)
    // 如果未定义，可以使用 Tailwind 的标准颜色，例如 bg-gray-700, text-gray-200 等
    const defaultBg =
      "bg-vscode-button-secondaryBackground hover:bg-vscode-button-secondaryHoverBackground";
    const defaultText = "text-vscode-editor-foreground";
    const defaultBorder = "border-vscode-contrastBorder";

    const selectedBg =
      "bg-vscode-button-background hover:bg-vscode-button-hoverBackground"; // 例如蓝色调
  ...
  return (
    <div className="bg-vscode-editor-background text-vscode-editor-foreground mt-2 w-full rounded-md border p-4">
      <p className="mb-6 text-base font-semibold">{question}</p>{" "}
      {/* 调整了 mb 和 text 大小 */}
      ...
      <div className="mt-4 flex justify-end">
        <button
          ...
          className="bg-vscode-button-background text-vscode-button-foreground hover:bg-vscode-button-hoverBackground flex items-center rounded-md p-2 text-sm disabled:opacity-50" // 使用vscode主题颜色
        >
