...
async function dynamicImportAndActivate(context: vscode.ExtensionContext) {
  ...
  process.on("uncaughtException", (error) => {
    if (
      error.name === "UriError" ||
      error.message.includes("Scheme contains illegal characters")
    ) {
      console.warn("[CA:Ext] Suppressed UriError:", error.message);
      return;
    }
    ...
    if (
      ...
      console.warn(
        "[CA:Ext] Suppressed network connection error:",
        error.message,
      );
      return;
    }
    ...
    if (
      ...
      console.warn(
        "[CA:Ext] Suppressed network error:",
        (error as any).code,
        error.message,
      );
      return;
  ...
  process.on("unhandledRejection", (reason, promise) => {
    ...
    if (error && typeof error === "object") {
      ...
      if (
        ...
        (error.message &&
          error.message.includes("GitHub.vscode-pull-request-github"))
      ) {
        console.warn(
          "[CA:Ext] Suppressed unhandled network rejection:",
          error.message || error,
        );
        return;
      }
    ...
    console.warn("[CA:Ext] Unhandled Promise Rejection:", reason);
  });
...
export function activate(context: vscode.ExtensionContext) {
  return dynamicImportAndActivate(context).catch((e) => {
    console.log("[CA:Ext] Error activating extension: ", e);
